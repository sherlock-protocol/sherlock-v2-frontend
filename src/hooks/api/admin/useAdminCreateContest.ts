    ...
    async (params) => {
      try {
        await contestsAPI.post(adminCreateContestUrl(), {
          ...
          starts_at: params.contest.startDate.set({ hour: 15, minute: 0 }).toSeconds(),
          ends_at: params.contest.endDate.set({ hour: 15, minute: 0 }).toSeconds(),
          judging_ends_at: params.contest.judgingEndDate.set({ hour: 15, minute: 0 }).toSeconds(),
